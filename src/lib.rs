...
const PHI_CHALLENGE_COLOR_RAINBOW: i32 = 5;
const PHI_AUDIO_MIX_MODE_OPTIMIZED: i32 = 1;

const fn phi_string_view(value: &'static str) -> phi_string_view_t {
    phi_string_view_t {
        data: value.as_ptr(),
        length: value.len(),
    }
...
unsafe fn copy_utf8(value: phi_string_view_t) -> Result<String, phi_status_t> {
    if value.length == 0 {
...
fn validate_header(struct_size: u32, abi_version: u32, expected_size: usize) -> phi_status_t {
...
fn ffi_status(function: impl FnOnce() -> phi_status_t) -> phi_status_t {
...
#[allow(dead_code)]
fn set_error(context: &phi_context, message: impl Into<String>) {
